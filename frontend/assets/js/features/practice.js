...
import { esc, icon, routeLink, progressBar, shuffle, audioButton } from "../core/ui.js";
...
export function renderPractice(ctx, options) {
  ...
  function setChoices() {
    ...
    choices = shuffle([item.answer, ...shuffle(others).slice(0, 3)]);
  }
  function draw() {
  ...
  ctx.main.addEventListener("click", event => {
    ...
    if (action === "next" && feedback) { index++; feedback = null; setChoices(); draw(); }
    if (action === "retry") {
