...
export function renderLibrary(ctx) {
  ...
  function drawResults() {
    ...
    ctx.main.querySelector("#library-results").innerHTML = list.length ? list.map(item => `<article class="panel library-card"><span class="pill">${item.level} · ${CAT_LABEL[item.cat]}</span><h2 class="jp" lang="ja">${esc(item.term)}</h2><p class="romaji" lang="ja">${esc(item.reading)}</p><h3>${esc(item.meaning)}</h3><div class="library-example"><p class="jp" lang="ja">${esc(item.example)} ${audioButton(item.example)}</p><p class="small" lang="ja">${esc(item.exampleReading)}</p><p>${esc(item.exampleMeaning)}</p></div>${addButton(ctx, item.id)}</article>`).join("") : emptyState("Nenhum resultado", "Experimente mudar o nível, o tipo ou a busca.");
  }
