...
export function renderLesson(ctx, id) {
  ...
  function draw() {
    if (step < lesson.sections.length) {
      ...
      shell(`<span class="step-label">ENTENDA · ${step + 1} DE ${lesson.sections.length}</span><h2 data-focus tabindex="-1">${section.title}</h2><p class="lesson-body">${section.body}</p><div class="examples-grid">${section.examples.map(example => exampleHTML(example, ctx.progress.preferences.romaji)).join("")}</div>${section.tip ? `<aside class="tip-box">${icon("spark")}<p>${section.tip}</p></aside>` : ""}${help}<div class="lesson-controls"><button class="btn btn-ghost" data-lesson="back" ${step ? "" : "disabled"}>${icon("back")} Voltar</button><span class="small muted">Leia, ouça e experimente.</span><button class="btn btn-primary" data-lesson="next">${step + 1 === lesson.sections.length ? "Praticar o que aprendi" : "Continuar"} ${icon("arrow")}</button></div>`);
