...
export function renderLesson(ctx, id) {
  ...
  function draw() {
    if (step < lesson.sections.length) {
      ...
      const help = concepts.length ? `<details class="concept-help"><summary>Em outras palavras · termos desta explicação</summary><dl>${concepts.map(item => `<dt>${item.term}</dt><dd>${item.definition}<small>${item.example}</small></dd>`).join("")}</dl></details>` : "";
      shell(`<span class="step-label">ENTENDA · ${step + 1} DE ${lesson.sections.length}</span><h2 data-focus tabindex="-1">${section.title}</h2><p class="lesson-body">${section.body}</p><div class="examples-grid">${section.examples.map(example => exampleHTML(example, ctx.progress.preferences.romaji)).join("")}</div>${section.tip ? `<aside class="tip-box">${icon("spark")}<p>${section.tip}</p></aside>` : ""}${help}${capsule ? `<aside class="culture-capsule"><p class="eyebrow">JAPONÊS EM CONTEXTO</p><h3>${capsule.title}</h3>${exampleHTML(capsule.expression, ctx.progress.preferences.romaji)}<p>${capsule.expression.context}</p>${routeLink("expressions", "Conhecer mais expressões " + icon("arrow"), "text-link")}</aside>` : ""}<div class="lesson-controls"><button class="btn btn-ghost" data-lesson="back" ${step ? "" : "disabled"}>${icon("back")} Voltar</button><span class="small muted">Leia, ouça e experimente.</span><button class="btn btn-primary" data-lesson="next">${step + 1 === lesson.sections.length ? "Praticar o que aprendi" : "Continuar"} ${icon("arrow")}</button></div>`);
